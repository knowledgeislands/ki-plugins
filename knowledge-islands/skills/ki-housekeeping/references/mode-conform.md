...
_On-demand procedure for `ki-housekeeping` CONFORM. The domain and auto-memory model live in [`SKILL.md`](../SKILL.md) and its standards._

1. Run [AUDIT](mode-audit.md).
2. Run `ki repo conform --skill ki-housekeeping` to propose only the two safe memory repairs: aligning a frontmatter name with its contained physical filename and appending an unindexed contained memory file to an existing physical MEMORY.md. The host validates and publishes the shared-session proposal.
