...
const SELF_SOURCE = join('.agents', 'skills', 'ki-self', 'SKILL.md')
const CLAUDE_PROJECTION = join('.claude', 'skills', 'ki-self')
const EXPECTED_CLAUDE_PROJECTION = '../../.agents/skills/ki-self'
...
export type HousekeepingSelfContext = {
  source: readonly AuditOutcome[]
  sourceName: readonly AuditOutcome[]
  projection: readonly AuditOutcome[]
}
...
const unavailableSelfContext = (subject: string): HousekeepingSelfContext => {
  const outcome = notApplicable('Repository-local evidence is not selected for this memory subject.', subject)
  return { source: outcome, sourceName: outcome, projection: outcome }
}

const repositorySelfContext = (repository: string): HousekeepingSelfContext => {
  const sourcePath = join(repository, SELF_SOURCE)
  const sourceState = existsSync(sourcePath)
    ? lstatSync(sourcePath).isFile() && !lstatSync(sourcePath).isSymbolicLink()
      ? 'present'
      : 'invalid'
    : 'missing'
  const source =
    sourceState === 'missing'
      ? one({ status: 'VIOLATION', message: 'missing repository-local ki-self source', subject: SELF_SOURCE })
      : sourceState === 'invalid'
        ? one({
            status: 'VIOLATION',
            level: 'FAIL',
        ...
        : one({ status: 'PASS', message: 'repository-local ki-self source is present', subject: SELF_SOURCE })
  const sourceName =
    sourceState !== 'present'
      ? notApplicable('No present repository-local source to check.', SELF_SOURCE)
      : one(
          /^name:\s*ki-self\s*$/m.test(readFileSync(sourcePath, 'utf8'))
            ? { status: 'PASS', message: 'repository-local skill source declares name: ki-self', subject: SELF_SOURCE }
            : { status: 'VIOLATION', message: 'repository-local skill source must declare name: ki-self', subject: SELF_SOURCE }
        )
  const configPath = join(repository, '.ki-config.toml')
  const runtimes = physicalFile(configPath) ? configuredRuntimes(readFileSync(configPath, 'utf8')) : []
  const projectionPath = join(repository, CLAUDE_PROJECTION)
  const projection = !runtimes.includes('claude-code')
    ? notApplicable('Claude Code is not a declared runtime.', '.ki-config.toml')
    : one(
        existsSync(projectionPath) &&
          lstatSync(projectionPath).isSymbolicLink() &&
          readlinkSync(projectionPath) === EXPECTED_CLAUDE_PROJECTION
          ? { status: 'PASS', message: 'Claude Code projects the canonical ki-self source', subject: CLAUDE_PROJECTION }
          : {
              status: 'VIOLATION',
              message: 'Claude Code must project the canonical ki-self source by relative link',
              subject: CLAUDE_PROJECTION
            }
      )
  return { source, sourceName, projection }
}

const unavailableMemoryContext = (subject: string): HousekeepingRubricContext => {
  const memory = notApplicable('No physical Claude project memory directories under ~/.claude/projects.', subject)
  return {
    self: unavailableSelfContext(subject),
    index: {
      exists: memory,
...
const projectContext = (
  ...
  const memoryRoot = join('.claude', 'projects', slug, 'memory')
  ...
  const learnedEntries =
    slug !== repositorySlug
      ? notApplicable('This memory directory does not represent the selected repository.', memoryRoot)
      : index === null
        ? notApplicable('MEMORY.md is absent.', indexRelativePath)
        : start === -1 || end === -1 || end < start
  ...
  return {
    self: unavailableSelfContext(memoryRoot),
    index: {
      exists: one(
        index === null
...
export const createHousekeepingSession = ({
  ...
  const projects =
    physicalDirectory(claudeRoot) && physicalDirectory(projectsRoot)
      ? readdirSync(projectsRoot, { withFileTypes: true })
          .filter((entry) => entry.isDirectory() && !entry.isSymbolicLink())
          .flatMap((entry) => {
            const memoryDirectory = join(projectsRoot, entry.name, 'memory')
            return physicalDirectory(memoryDirectory)
              ? [
                  {
                    slug: entry.name,
                    context: projectContext(home, repositoryName, repositorySlug, entry.name, memoryDirectory, mode === 'conform', drafts)
                  }
                ]
              : []
          })
  ...
  const unavailable = unavailableMemoryContext(repositoryRoot)
  const repositoryContext: HousekeepingRubricContext = {
    self: repositorySelfContext(repositoryRoot),
    index: unavailable.index,
    frontmatter: unavailable.frontmatter,
  ...
  const memorySubjects =
    projects.length > 0
      ? projects.map((project) => ({
          ...
          subject: join('.claude', 'projects', project.slug, 'memory'),
          context: () => project.context
        }))
      : [
          {
            families: memoryFamilies,
            subject: join('.claude', 'projects'),
            context: () => unavailableMemoryContext(join('.claude', 'projects'))
          }
        ]

  return {
    subjects: [...memorySubjects, { families: ['SELF'], subject: repositoryRoot, context: () => repositoryContext }],
    proposal: () => {
