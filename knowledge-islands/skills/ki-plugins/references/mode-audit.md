...
_On-demand procedure for `ki-plugins` AUDIT. The exact repository and manifest contract lives in [the plugin-marketplace standard](standards-plugin-marketplace.md)._

1. Confirm the target repository path, defaulting to the current repository.
2. Run `ki repo audit --repo <repo> --skill ki-plugins`. The session inspects only physical repository paths, never follows symlinked projection or scaffold evidence, and publishes no writes or commands.
