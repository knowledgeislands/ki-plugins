...
- **Surfaces sit on a controllability ladder** (design record, most-controllable first): **Claude Code** and **Claude Desktop** are file-editable and already rendered by chezmoi — this skill audits them. **Claude Cowork** is file-editable in principle (`cowork_settings.json`, `enabledPlugins`) but gated on a launch-honours-external-edit check before anything depends on it. **claude.ai web** has no local file — documented convention only, no build.
- **The Cowork artifact is a KI plugin** (Claude plugin-marketplace format bundling servers + skills + agents). The plugin is the packaging; **this skill is the actor** that toggles it — the fan-out home is a skill, not a marketplace repo. Design: [cross-surface-enablement.md](../ki-mcp/references/cross-surface-enablement.md).
...
4. **Cowork** is gated: do not write `enabledPlugins` until the launch-honours-external-edit check passes (REFRESH / the design record §). Until then, CONFORM stops at the file-editable surfaces and names Cowork as pending.
