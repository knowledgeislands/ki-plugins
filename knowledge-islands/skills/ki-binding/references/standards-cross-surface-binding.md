...
2. **For each surface token `S`**, the expected server set is `{ e.name for e in mcpServers if S in e.clients }`.
...
4. **The skill half** — project-local skills for the surface — is not in `mcps.yaml`; it is `.ki-config.toml` coverage. BIND-3 checks that each declared skill exists in every supported runtime root; `ki-bootstrap` owns conforming those publications.

## Invariants

- **One source.** No MCP surface config is authored by hand; each is rendered from `mcps.yaml` by a renderer (the file-editable surfaces — `ki-binding-chezmoi` on the maintainer's machine, or any tool reading the canonical source). Cowork plugin registration is a separate binding derived from the fixed KI marketplace identity. A hand-edit that diverges from either contract is drift, reported by BIND-1 or BIND-4.
