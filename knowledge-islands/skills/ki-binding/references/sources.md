...
| --- | --- | --- | --- |
| [DR] | `cross-surface-enablement.md` design record owned by `ki-mcp` | Per-surface controllability, home decision, build sequence † | 2026-07-06 |
| [SRC] | `~/.config/ki/mcp-servers.yaml` (canonical; resolution order in [the standard](standards-cross-surface-binding.md)) | The single source this skill reads and audits against ‡ | 2026-07-13 |
...
- **New surfaces / `clients` tokens** — if a renderer adds a rendered surface, extend the session's recognised surfaces and the recognised-surfaces table in the standard.
