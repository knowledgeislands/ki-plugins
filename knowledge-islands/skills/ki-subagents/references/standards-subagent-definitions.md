...
- **Progress visibility**: a coordinator owns user-visible progress for long-running or background work. It tells the caller the next expected checkpoint before dispatch, reports phase completion and material blockers, and defaults to a concise five-minute cadence when the caller has not specified one. Workers report interim state to the coordinator; they do not independently substitute for the coordinator's updates. When `ki-delegation` is active in the same scope, use its durable packet standard. (HOUSE)
