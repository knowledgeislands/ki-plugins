...
const temporaryRoot = (): string => {
  const root = mkdtempSync(join(tmpdir(), 'ki-repo-plugins-session-'))
  roots.push(root)
  return root
}
...
const context = (session: ReturnType<typeof createPluginsSession>) => {
  ...
  if (!subject) throw new Error('ki-repo-plugins session did not expose a subject')
  return subject.context()
}

describe('ki-repo-plugins session', () => {
