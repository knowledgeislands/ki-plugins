...
const unavailable = (context: PluginsContext): readonly AuditOutcome[] | null =>
  !context.available
    ...
    : !context.applicable
      ? [{ status: 'NOT_APPLICABLE', message: 'ki-repo-plugins is not applicable to this repository' }]
      : null
...
const PLUG_15 = mechanical(
  ...
  'Applicable repositories declare `[skills.ki-repo-plugins]` and no unknown keys.',
  'WARN',
  (context) =>
    active(context, () =>
      result(
