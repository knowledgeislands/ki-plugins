...
- `name` — the marketplace name (`ki-repo-plugins`).
...
`ki-binding-claude` owns **generation** (`build-plugin.ts`) and **cross-surface enablement** (its checker verifies that the machine's `cowork_settings.json` points at the repo and enables the plugin). `ki-repo-plugins` owns the **repo's on-disk shape**. The two do not re-check each other's half. This is an ownership and coverage boundary, not a dependency edge.
