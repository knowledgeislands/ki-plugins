...
- **One governance-mode model (SHAPE-5).** The universal INIT/AUDIT/CONFORM/REFRESH plus skill-specific modes, so a new skill inherits the shape. INIT's mechanical half is the per-skill `scripts/bootstrap.ts` (§5).
