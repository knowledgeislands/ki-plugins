...
- Skill-id lag: `ki-arcadia-principal`'s CLAUDE.md and process note reference `knowledgeislands-streams`; the harness skill is now `ki-repo-kb-streams`. Tracked centrally under the `ki-repo-kb-*` rename, not a model change.
