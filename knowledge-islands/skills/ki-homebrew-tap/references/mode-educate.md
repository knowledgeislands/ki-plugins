...
4. **Wire governance.** Add `[ki-repo]` + `[ki-homebrew-tap]` to `.ki-config.toml`—after `ki-repo` establishes the shared file, `ki repo conform --skill ki-homebrew-tap --repo <tap-path>` adds the keyless `[ki-homebrew-tap]` marker safely. Run `ki-repo`'s EDUCATE for the repo-level files and GitHub settings. Do **not** add `[ki-engineering]`—a tap has no TypeScript toolchain (the `ki-plugins` precedent).
