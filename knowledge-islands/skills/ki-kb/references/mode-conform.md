...
1. Run `ki repo conform --skill ki-kb --repo <base> --dry-run`. The current safe actions propose missing same-name zone indexes and the root `Admin/MEMORY.md` from session-owned drafts; the host validates and publishes the transaction.
