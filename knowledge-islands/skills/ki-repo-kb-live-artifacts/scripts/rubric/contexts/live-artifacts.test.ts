...
const repository = (): string => {
  const root = mkdtempSync(join(tmpdir(), 'ki-repo-kb-live-artifacts-session-'))
  roots.push(root)
  return root
}
...
const rootContext = (session: ReturnType<typeof createLiveArtifactsSession>) => {
  ...
  if (!subject) throw new Error('ki-repo-kb-live-artifacts session did not expose its repository subject')
...
describe('ki-repo-kb-live-artifacts session', () => {
