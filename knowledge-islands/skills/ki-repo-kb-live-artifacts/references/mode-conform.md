...
2. Run `ki repo conform --skill ki-repo-kb-live-artifacts` to propose creating or appending unambiguous index entries and adding a missing `renders: html` declaration to an existing frontmatter block. It does not infer a missing `status` or `author`.
