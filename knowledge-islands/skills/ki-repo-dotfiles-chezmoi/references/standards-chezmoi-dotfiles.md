...
**Review a change scoped, and know which command recurses.** `chezmoi diff` over a whole source tree renders every managed target and can take minutes on a large one, so review a change by passing the target paths it touches. Two commands answer different questions and disagree on directory arguments: `chezmoi status <dir>` recurses by default and lists every changed target beneath it, while `chezmoi diff <dir>` prints _nothing at all_ without `-r`/`--recursive` — it reports on the directory entry itself, which is usually unchanged, and exits 0. An empty scoped `diff` is therefore not evidence that a directory is clean. Measured on chezmoi v2.72.2. Use `chezmoi status <dir>` for the inventory of what will change and `chezmoi diff -r <dir>` for the content, and note that a shell glob is not a substitute: chezmoi resolves arguments against managed targets, so an unexpanded `'~/dir/*'` fails with `not managed`.
