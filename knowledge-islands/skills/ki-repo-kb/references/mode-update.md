# Mode UPDATE — enrich an existing note

_On-demand procedure for kb's UPDATE mode. The shared model — the five-zone structure, routing test, memory cascade, project bindings, and Step 1 (Load context) — lives in [`SKILL.md`](../SKILL.md) and is already loaded; this file is the procedure only._

_Gate: if the note lives in a canonical zone and the base runs the Enactment Process, route the change through a proposal (`ki-repo-kb-streams`) rather than a direct write — unless it is a trivial fix or an unambiguous profile auto-append._

1. Run pre-flight.
2. Find and read the existing note.
3. Draft a merged version - enrich, do not replace. Preserve structure.
4. For profile / person notes: auto-append unambiguous factual updates from authoritative sources; propose a diff first for tactical or evaluative content.
5. Cite the source of every update; confirm, then write.
