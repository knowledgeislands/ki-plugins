...
1. Run `ki repo audit --skill ki-repo-kb --repo <base>`. The catalogue prepares each focused family context once and the host reports the typed outcomes without changing the base.
...
3. Confirm that the host ran the declared focused prerequisites for Activities, Live Artifacts, and Streams before `ki-repo-kb`. Run separately declared coverage such as `ki-authoring` for Markdown conventions. A base is clean only when every applicable governing skill is clean.
