...
_On-demand procedure for ki-repo-website's AUDIT mode. The hosted rubric inspects the site-build delta; separate skills and explicit build or hosting checks remain distinct._
...
2. **Run the mechanical rubric.** Run `ki repo audit --repo <repo> --skill ki-repo-website`. It inspects the Eleventy dependency and configuration, config-less Tailwind entry, site source layout, script family, SEO partial, portable output seam, generated-output ignore, and `[skills.ki-repo-website]` opt-in. Capture its output verbatim.
