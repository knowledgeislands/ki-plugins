...
1. **Apply the safe hosted repairs.** Run `ki repo conform --repo <repo> --skill ki-repo-website`. On physical, contained files the session may add the keyless `[skills.ki-repo-website]` marker and correct the generated-output ignore entry to `site/dist`. Missing files may be proposed for creation; malformed or symlinked configuration and unsafe paths remain report-only.
