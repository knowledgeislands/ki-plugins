...
_On-demand procedure for ki-repo-website's EDUCATE mode. Scaffolding creates application source and therefore remains deliberate authoring rather than a hosted rubric repair._
...
4. **Declare governance.** Add the keyless `[skills.ki-repo-website]` table to `.ki-config.toml` and ignore `site/dist/`.
5. **Verify before handoff.** Run `ki repo audit --repo <repo> --skill ki-engineering`, `ki repo audit --repo <repo> --skill ki-repo-website`, and the site's build. If Cloudflare will serve the output, use `ki-repo-website-cloudflare` separately for that layer.
