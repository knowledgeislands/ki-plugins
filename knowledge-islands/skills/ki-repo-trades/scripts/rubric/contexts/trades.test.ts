...
const fixture = (peerIdentity = 'peer/repo', reciprocal = true) => {
  const home = temporaryDirectory('ki-repo-trades-home-')
  const local = temporaryDirectory('ki-repo-trades-local-')
  const peer = temporaryDirectory('ki-repo-trades-peer-')
  scaffold(local)
  scaffold(peer)
...
test('outbound records are valid on a declared export route while receiver participation is pending', () => {
  ...
  expect(mechanicalOutcomes(session, ROUTE)).toContainEqual({
    status: 'INFO',
    message: 'declared export route https://github.com/peer/repo awaits receiver ki-repo-trades participation',
    subject: 'https://github.com/peer/repo'
  })
