...
const CONFIG_TABLE = 'ki-repo-trades'
...
const parseConfiguration = (
  ...
  for (const key of unknown)
    outcomes.push({
      status: 'VIOLATION',
      level: 'WARN',
      message: `unrecognised ki-repo-trades configuration key ${key}`,
      subject
    })
...
const routeEvidence = (
  ...
  const validateRoute = (peer: string, direction: 'export' | 'import', kind: TradeKind): void => {
    ...
    if (!candidate?.configuration.participates) {
      outcomes.push({
        status: 'INFO',
        message: `declared ${direction} route ${peer} awaits ${otherParty} ki-repo-trades participation`,
        subject: peer
      })
      return
    }
    if (!candidate.configuration.valid) {
      outcomes.push({
        status: 'VIOLATION',
        message: `registered route endpoint ${peer} has malformed ki-repo-trades configuration`,
        subject: peer
      })
      return
    }
...
const scaffoldEvidence = (root: string): readonly AuditOutcome[] => {
  ...
  for (const readme of TRADE_READMES) {
    ...
    if (!containedPhysical(root, directory, 'directory'))
      ...
      outcomes.push({
        status: 'VIOLATION',
        message: `${readme.path} differs from the canonical ki-repo-trades orientation`,
        subject: readme.path
      })
  }
  return outcomes
