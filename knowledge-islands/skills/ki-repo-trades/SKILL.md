...
1. **Declared participation** — a repository opts in with its own `ki-repo-trades` table, declaring typed `exports_to` and `imports_from` routes. Its canonical HTTPS GitHub home comes from `ki-repo.repository`.
...
Run `ki repo audit --skill ki-repo-trades --repo <repo>`. The structured catalogue validates local configuration, declared and active typed routes, owned scaffold, preparation and submitted-record shape, sender/receiver authority, receipt and decision fields, raw sender-projection agreement, and observation-led release or pruning. Then review whether any direct application, local adoption, or knowledge retention preserves the receiver's independent authority.

### Mode CONFORM

Run AUDIT first. `ki repo conform --skill ki-repo-trades --repo <repo> --dry-run` may restore only the owned `_TRADES` README scaffold when the generic `+` and `-` areas are safe physical directories. It never creates a route, record, receiver copy, disposition, roadmap item, or cross-repository write. Apply authored record and configuration corrections locally, then re-run AUDIT.

### Mode EDUCATE

Run `ki repo educate --skill ki-repo-trades --repo <repo>` to render the concern and rubric. To participate, declare `ki-repo.repository`, then typed routes in `.ki-config.toml`, ensure the generic working areas exist through `ki-repo`, and scaffold only this skill's `_TRADES` README files. EDUCATE grants no peer authority and creates no trade record.
...
- The checker reads only registered repository roots and their public `ki-repo-trades` declarations and records. It never scans for repositories or writes a peer checkout.
