...
Each partner appears exactly once: TOML's own prohibition on defining a key twice enforces that, so no hand-written uniqueness or lexical-ordering rule is needed. `[skills.ki-repo-trades]` is declared explicitly rather than implied by its `routes` sub-table, because declaring a skill is separate from configuring it.
...
The generic `+` and `-` working areas remain owned by `ki-repo`. A repository declaring `ki-repo-trades` also carries:
