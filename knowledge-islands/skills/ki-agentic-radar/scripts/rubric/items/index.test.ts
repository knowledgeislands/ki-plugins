...
test('the catalogue exposes the complete ordered agentic-radar contract', () => {
  expect(definition.contract).toBe(1)
  expect(definition.name).toBe('ki-agentic-radar')
...
test('family modules keep narrow public surfaces', async () => {
  ...
  for (const file of ['schema', 'evidence', 'classification', 'lifecycle', 'publication']) {
