...
1. **Files** — every repo carries `README.md`, `LICENSE`, `.gitignore`, and `.ki.toml` (its declared config). A local target reads its checkout first; an `--org` or other filesystem-free run reads the GitHub default branch. (`.editorconfig` is owned by `ki-authoring`, not this skill.)
