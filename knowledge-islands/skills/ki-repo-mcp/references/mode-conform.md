...
3. **Apply bounded MCP repairs.** Run `ki repo conform --skill ki-repo-mcp --repo <repo-path>`. The session may append `[skills.ki-repo-mcp]` to an existing valid regular `.ki.toml` and normalise MCP `main`, `bin`, and required `exports` in an existing valid regular `package.json`. Missing, malformed, symlinked, or otherwise unsafe targets remain report-only.
