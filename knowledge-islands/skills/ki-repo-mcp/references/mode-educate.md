...
4. Run `ki repo audit --skill ki-repo-mcp --repo <repo>`, then run `bun run test` explicitly (NOT `bun test`). When the repo carries `vitest.config.*`, its configured coverage gate must also pass.
