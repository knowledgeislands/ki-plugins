...
`ki-repo-trades` owns the record, route, authority, and lifecycle contract. This skill runs its audit before and after a mutation and uses the host command as the transaction boundary.

## Responsibility boundary

`ki-repo-trade` never sets a receiver disposition. It hands a validated inbound record to `ki-next`, which records the exact human-confirmed decision and chooses between a direct local application and a separately confirmed roadmap proposal.
...
The remaining invocations map one-to-one to the local `ki trade` surface. Mutating operations resolve the physical repository root, run the `ki-repo-trades` audit, present the exact local write or deletion, obtain any confirmation required by [the procedure](references/standards-trade-operations.md), execute once, and re-audit.
