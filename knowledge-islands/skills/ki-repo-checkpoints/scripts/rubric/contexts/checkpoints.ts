...
const CONFIG_TABLE = 'ki-repo-checkpoints'
...
export const createCheckpointsSession = ({
  ...
  const configOutcomes: AuditOutcome[] = configured
    ? Object.keys(configured).map((key) => ({
        ...
        level: 'WARN' as const,
        message: `unrecognised ki-repo-checkpoints configuration key ${key}`,
  ...
  const context: CheckpointsRubricContext = {
    ...
    configuration: {
      outcomes: one(
        configOutcomes,
        configured
          ? 'The ki-repo-checkpoints declaration is an empty capability marker.'
          : 'No ki-repo-checkpoints declaration is active.'
      )
    },
    structure: {
