...
1. Run `ki repo conform --skill ki-repo-kb-activities`. The hosted action proposes one safe `Activities.md` create or update when activity notes exist and index entries are missing; the host owns dry-run, publication, rollback, and post-conform audit.
