...
An **Agora** is a named, portable collection of independently governed Knowledge Islands repositories. It is neither a filesystem directory nor a client workspace. Its home approves the repositories that may participate, and each member repository independently records its consent.
...
The home repository does not list itself in `members`: operating the home is its distinct authority, not implicit member consent. A different repository may operate another Agora and also join this one.
...
A local resolver may report a membership as reciprocal only when all of the following are locally resolvable:

1. the member declaration's `home` identifies one registered repository;
